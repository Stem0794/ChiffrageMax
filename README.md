...
> **Security**: no secrets are stored in the repository. By default, the **OAuth Client ID** and **template Sheet ID** are **not baked in** (empty `BAKED` in `js/config.js`): each user enters them via Settings, or the deployer embeds them in their own private copy. A web OAuth Client ID is *not* a secret (it travels in plaintext in every OAuth request), but baking it in routes all visitors' OAuth traffic and API quota through **your** Google Cloud project — only do this for a controlled internal deployment. Configuration is synchronized through the signed-in user's Drive; account-scoped browser data is cleared on sign-out.
...
- **Inline cell editing**: quote number, client (dropdown), project, date (picker), ticket. The Drive file is **automatically renamed** when the project or date changes; it is **moved** to the correct `year/month` folder when the client or date changes.
...
- **Archive**: the archive button prepends `[ARCH]` to the filename, which is then skipped on future scans (zero API calls).
- **Delete**: the delete button permanently deletes the Google Sheet when permitted; otherwise it moves it to the Drive trash.
...
- **Calendar button on each dashboard row**: opens a panel to set date ranges for 4 phases (**Design**, **Development**, **Testing**, **Go-Live**). End date is optional (e.g. Go-Live shows only a start milestone).
...
On first launch, open **Configuration** and follow the three numbered sections. Nothing is baked in by default. For an internal deployment, you can **bake in** your values in `js/config.js` (`BAKED.clientId` / `BAKED.templateId`) so teammates have **nothing to configure**.

### Quick start for a new user

1. In **Connexion Google**, paste the OAuth **Client ID** from Google Cloud. Enable both the Google Drive and Google Sheets APIs, and add the app URL to **Authorized JavaScript origins**.
2. In **Modèle de chiffrage**, paste the template Google Sheet URL or ID. Share the template with the account that will sign in (read-only is enough).
3. In **Clients et dossiers Drive**, add a client and paste its Drive folder URL or ID. A root folder can be added as a fallback, but is not required when every client has its own folder.
4. Click **Enregistrer**, then **Se connecter avec Google**. The first Google consent screen is expected; accept Drive and Sheets access.

If a teammate already created the shared configuration, paste its ID in **Synchronisation avec l'équipe** instead of entering the template and clients manually. You still need your Google OAuth Client ID to sign in.
...
- **Clients**: for each client, a name and a root Drive folder. The day-rate button lets you configure roles and day rates per client.
